...
def calculate_score(data: Dict) -> Tuple[int, str]:
    ...
    # 在 calculate_score 裡加
    if foreign_days >= 3 and trust_days >= 3:
        breakdown.dual_institution = 15
        reasons.append(f"法人共振 (+15)")
    turnover_rate = data.get("turnover_rate", 0) or 0
    if 0.5 <= turnover_rate <= 8.0:  # 健康週轉率區間
        breakdown.healthy_turnover = 5
        reasons.append(f"週轉率健康 (+5)")
    price_3d_trend = data.get("price_3d_trend", 0) or 0  # 近3日漲幅%
    if price_3d_trend > 3:
        breakdown.price_momentum = 10
        reasons.append(f"3日動能+{price_3d_trend:.1f}% (+10)")
    foreign_ratio_change = data.get("foreign_ratio_change", 0) or 0
    if foreign_ratio_change > 0.5:  # 持股比上升0.5%以上
        breakdown.foreign_ratio_up = 10
        reasons.append(f"外資持股比↑ (+10)")
