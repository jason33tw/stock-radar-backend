...
# 自動判斷資料庫類型，轉換成對應的 async driver URL
# - SQLite (本機開發用): sqlite:/// → sqlite+aiosqlite:///
# - PostgreSQL (Render 等雲端平台): postgres:// 或 postgresql:// → postgresql+asyncpg://
_raw_url = settings.DATABASE_URL

if _raw_url.startswith("postgres://"):
    # Render 給的 URL 開頭是 postgres://，要轉成 postgresql:// 才能被 SQLAlchemy 辨識
    _raw_url = _raw_url.replace("postgres://", "postgresql://", 1)

if _raw_url.startswith("postgresql://"):
    db_url = _raw_url.replace("postgresql://", "postgresql+asyncpg://", 1)
elif _raw_url.startswith("sqlite:///"):
    db_url = _raw_url.replace("sqlite:///", "sqlite+aiosqlite:///", 1)
else:
    db_url = _raw_url
